...
namespace LoginSIME.Services
{
    public class Request
    {
        ...
        public async Task<TResponse> PostLoginAsync<TBody, TResponse>(string uri, TBody data)
             where TBody : class
             where TResponse : class
        {
            ...
            if (response.StatusCode == System.Net.HttpStatusCode.OK)
            {
                return JsonConvert.DeserializeObject<TResponse>(serialized);
            }
            else
            {
                // Se houver erro, jogue a exceção contendo a mensagem de erro do servidor
                throw new Exception(serialized);
            }
        }
